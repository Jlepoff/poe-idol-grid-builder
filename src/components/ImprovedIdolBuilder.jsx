...
function ImprovedIdolBuilder({ modData, idolTypes, onAddIdol }) {
  ...
  const handleCreateIdol = () => {
    ...
    const name = idolName || generateIdolName(selectedType, selectedPrefixes, selectedSuffixes);
              ...
              {selectedPrefixes.length > 0 ? (
                <ul className="mb-3 space-y-1">
                  {selectedPrefixes.map((prefix, index) => (
                    <li key={index} className="bg-gray-700 p-2 rounded flex justify-between">
                      <div>
                        <div className="text-sm font-medium">{prefix.Name}</div>
                        <div className="text-xs text-gray-300">{prefix.Mod}</div>
                      </div>
                      <button 
                  ...
                  {selectedSuffixes.map((suffix, index) => (
                    <li key={index} className="bg-gray-700 p-2 rounded flex justify-between">
                      <div>
                        ...
                        <div className="text-xs text-gray-300">{suffix.Mod}</div>
                      </div>
                      <button 
