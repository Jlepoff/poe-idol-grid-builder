...
function IdolItem({ idol, onRemoveIdol }) {
  ...
  return (
    <div 
      ref={drag}
      className={`${bgColor} p-3 rounded-lg shadow mb-2 cursor-move ${opacity} ${idol.isPlaced ? 'border-2 border-white' : ''}`}
          ...
          <div>
            ...
            <ul className="text-xs space-y-1">
              {idol.prefixes.map((prefix, idx) => (
                <li key={`prefix-${idx}`} className="text-gray-200">{prefix.Mod}</li>
              ))}
            </ul>
          </div>
        )}
          ...
          <div>
            ...
            <ul className="text-xs space-y-1">
              {idol.suffixes.map((suffix, idx) => (
                <li key={`suffix-${idx}`} className="text-gray-200">{suffix.Mod}</li>
              ))}
            </ul>
          </div>
        )}
          ...
          <div>
            ...
            <ul className="text-xs space-y-1">
              {idol.uniqueModifiers.map((mod, idx) => (
                <li key={`unique-${idx}`} className="text-gray-200">{mod.Mod}</li>
              ))}
            </ul>
          </div>
        )}
      ...
      {inventory.length > 0 ? (
        <>
          <div className="text-sm text-gray-400 mb-2">
            ...
            <span className="text-xs ml-1">Drag to place • Right-click to remove</span>
          </div>
        ...
        <p className="text-gray-400">No idols in inventory. Create an idol or paste one from Path of Exile.</p>
      )}
    </div>
  );
}
