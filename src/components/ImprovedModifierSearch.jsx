...
function ImprovedModifierSearch({ 
  ...
  const modifierNames = useMemo(() => {
    if (!modData.prefixes || !modData.suffixes) return { prefixes: [], suffixes: [] };
  ...
  const filteredModifiers = useMemo(() => {
    ...
    if (searchContext === 'autogen') {
      ...
      if (viewByName && nameFilter) {
        ...
        Object.entries(modData.suffixes).forEach(([typeName, suffixList]) => {
          suffixList.forEach(suffix => {
            if (suffix.Name === nameFilter || suffix.Name === `of ${nameFilter}`) {
      ...
      // When searching by text in auto-generate mode
        ...
        Object.entries(modData.prefixes).forEach(([typeName, prefixList]) => {
          prefixList.forEach(prefix => {
            if (prefix.Name.toLowerCase().includes(lowerTerm) || 
        ...
        Object.entries(modData.suffixes).forEach(([typeName, suffixList]) => {
          suffixList.forEach(suffix => {
            if (suffix.Name.toLowerCase().includes(lowerTerm) || 
    ...
    if (viewByName && nameFilter) {
      ...
      if (filterType === 'all' || filterType === 'prefix') {
        if (searchContext === 'builder' && selectedType && modData.prefixes[selectedType]) {
          // For type-specific filtering in builder context
          const filtered = modData.prefixes[selectedType].filter(mod => 
            mod.Name === nameFilter
          );
          ...
          // For all idol types
          Object.values(modData.prefixes).forEach(typeModifiers => {
            const filtered = typeModifiers.filter(mod => 
              mod.Name === nameFilter
            );
      ...
      if (filterType === 'all' || filterType === 'suffix') {
        if (searchContext === 'builder' && selectedType && modData.suffixes[selectedType]) {
          // For type-specific filtering in builder context
          const filtered = modData.suffixes[selectedType].filter(mod => 
    ...
    if (searchTerm) {
      ...
      if (filterType === 'all' || filterType === 'prefix') {
        ...
        if (searchContext === 'builder' && selectedType && modData.prefixes[selectedType]) {
          filteredPrefixes = modData.prefixes[selectedType].filter(prefix => 
          ...
          filteredPrefixes = allPrefixes.filter(prefix => 
            prefix.Name.toLowerCase().includes(lowerTerm) || 
            prefix.Mod.toLowerCase().includes(lowerTerm)
          );
        }
      ...
      if (filterType === 'all' || filterType === 'suffix') {
        ...
        if (searchContext === 'builder' && selectedType && modData.suffixes[selectedType]) {
          filteredSuffixes = modData.suffixes[selectedType].filter(suffix => 
          ...
          filteredSuffixes = allSuffixes.filter(suffix => 
            suffix.Name.toLowerCase().includes(lowerTerm) || 
            suffix.Mod.toLowerCase().includes(lowerTerm)
          );
        }
    ...
    return { prefixes: [], suffixes: [] };
  }, [searchTerm, filterType, modData, selectedType, viewByName, nameFilter, searchContext]);
  ...
  return (
    <div className="bg-gray-800 rounded-lg">
      ...
      <div className="flex space-x-4 mb-4">
        <button
          className={`px-3 py-1.5 rounded font-medium ${!viewByName ? 'bg-blue-600' : 'bg-gray-700'}`}
      ...
      {viewByName ? (
        <div className="mb-4">
          ...
          <div className="flex flex-wrap gap-2 max-h-40 overflow-y-auto">
            {/* Show prefix names */}
            {filterType !== 'suffix' && modifierNames.prefixes
              .filter(name => nameSearchTerm === '' || 
                             name.toLowerCase().includes(nameSearchTerm.toLowerCase()))
              .map(name => (
                <button
                  key={`prefix-name-${name}`}
                  className={`px-2 py-1 text-sm rounded ${nameFilter === name ? 'bg-blue-600' : 'bg-gray-700'}`}
            ...
            {/* Show suffix names */}
            {filterType !== 'prefix' && modifierNames.suffixes
              .filter(name => nameSearchTerm === '' || 
                             name.toLowerCase().includes(nameSearchTerm.toLowerCase()))
              .map(name => (
                <button
      ...
      <div className="results space-y-4">
        ...
        {(searchTerm || nameFilter) && 
          filteredModifiers.prefixes.length === 0 && 
          filteredModifiers.suffixes.length === 0 && (
          <p className="text-gray-400">No modifiers found matching "{searchTerm || nameFilter}"</p>
        )}
        
        {/* Help text when no search */}
        {!searchTerm && !nameFilter && (
          <p className="text-gray-400">Enter search terms or select a modifier name to see results</p>
        )}
      </div>
    </div>
  );
