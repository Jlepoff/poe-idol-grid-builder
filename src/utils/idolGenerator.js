...
export const generateIdols = (desiredModifiers, modData, idolTypes) => {
  ...
  const getAvailableIdolTypesGrouped = (modifier) => {
    ...
    directTypes.forEach(typeName => {
      ...
      if (equivType && directTypes.includes(equivType)) {
        ...
        const sizeKey = `${Math.min(idolType1.width, idolType1.height)}x${Math.max(idolType1.width, idolType1.height)}`;
    ...
    return {
      allTypes: result,
      groupedBySize: Object.values(groupedTypes).sort((a, b) => a.size - b.size)
    };
  };
  ...
  const getAvailableModifiers = (modType, idolTypeName) => {
    ...
    return uniqueModifiers
      .filter(mod => {
        ...
        if (mod.type === 'prefix') {
          // Check primary type
          const primaryCheck = modData.prefixes[idolTypeName] && 
                            modData.prefixes[idolTypeName].some(prefix => prefix.Code === mod.Code);
          
          // Check equivalent type
          const equivCheck = equivType && 
                           modData.prefixes[equivType] && 
                           modData.prefixes[equivType].some(prefix => prefix.Code === mod.Code);
                           
          return primaryCheck || equivCheck;
        } else {
          // Same for suffixes
          const primaryCheck = modData.suffixes[idolTypeName] && 
                            modData.suffixes[idolTypeName].some(suffix => suffix.Code === mod.Code);
          
          const equivCheck = equivType && 
                           modData.suffixes[equivType] && 
                           modData.suffixes[equivType].some(suffix => suffix.Code === mod.Code);
  ...
  while (unusedModifiers.length > 0 && !allModifiersUsed()) {
    ...
    const { allTypes, groupedBySize } = getAvailableIdolTypesGrouped(currentModifier);
    ...
    for (const sizeGroup of groupedBySize) {
      ...
      for (const idolTypeName of sizeGroup.types) {
        ...
        if (currentModifier.type === 'prefix') {
          ...
          const prefix = availablePrefixes.find(p => p.Code === currentModifier.Code);
          if (prefix) {
            selectedPrefixes.push(prefix);
            
            // Try to add a second prefix
            const otherPrefixes = availablePrefixes.filter(p => p.Code !== currentModifier.Code);
          ...
          const suffix = availableSuffixes.find(s => s.Code === currentModifier.Code);
          if (suffix) {
            selectedSuffixes.push(suffix);
            
            // Try to add a second suffix
            const otherSuffixes = availableSuffixes.filter(s => s.Code !== currentModifier.Code);
        ...
        if ((currentModifier.type === 'prefix' && selectedPrefixes.some(p => p.Code === currentModifier.Code)) ||
            (currentModifier.type === 'suffix' && selectedSuffixes.some(s => s.Code === currentModifier.Code))) {
          
          // Create idol with descriptive name
          const newIdol = {
            type: idolType.name,
            name: generateIdolNameWithDuplicates(idolType.name, selectedPrefixes, selectedSuffixes),
 ...
 */
export const generateAndPlaceIdols = (desiredModifiers, modData, idolTypes, currentGrid) => {
